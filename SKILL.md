...
- **首次/未配置**（`--check-auth` 报未配置时）：⚠️ **绝不要替用户写凭据文件、也不要让用户把 token
  贴进对话**——帮用户落地密钥是隐私敏感操作，别的用户的 Claude 会（应当）拒绝，这正是分发时卡住的原因。
  **正确做法：让用户自己在普通终端跑一条命令**：
    ```
    agent-orchestrate --setup-auth      # 或： python <本skill>/scripts/run.py --setup-auth
    ```
  这个向导会 `getpass` **隐藏输入** token/key、写入本机文件并收紧权限（含 `claude setup-token` 的引导），
  全程密钥不进对话/日志。你只需：① 告诉用户跑这条命令；② 用户跑完后再 `--check-auth` 确认已配置。
  订阅通道会在向导里提示先 `claude setup-token`。**你不碰密钥、不写文件。**
...
- **只配了一条**：直接用，不问。
- **订阅 + 中大型任务**：评审的 diff 已**默认裁剪到 40K 字符**（订阅友好、按验收相关性保留），
  所以**不要因为是订阅通道就关评审**——评审跑得起。更大的任务优先 `--decompose`（每个子任务
  评审更小）；想再省订阅额度可 `--review-model <小模型>` 让评审走便宜模型。**门越强，评审越轻越安全。**
...
| `--review-context-budget` | 评审输入(diff)裁剪字符预算（默认 40000，订阅友好）：超预算按验收相关性保留、其余只留统计。让评审在订阅额度下也跑得起，**不必因订阅而关评审** |
| `--review-model` | 只给评审用的 claude 模型（订阅上可用更省额度的小模型）；默认沿用 `--model` |
