...
- **子任务 DAG**（`--decompose`）：规划阶段把大需求拆成带 `deps` 的子任务，拓扑排序后逐个跑
  ...
  的下游被跳过**，独立分支照常推进。校验 id 唯一 / 依赖存在 / 无环。
- **最大轮数 + 预算**：双重防死循环 / 防失控成本（预算在整个 DAG 内共享）。
- **跨平台输出**：启动即把 stdout/stderr 重配为 UTF-8，避免 Windows GBK 控制台遇到
  `▶ ✅ 🎉` 等字符崩溃。
...
`codex exec … --full-access` 会跳过逐步确认、对工作区有完整读写执行权限。请只在**受控环境
/ 隔离仓库**里运行，并在提交前人工 review 所有改动。
