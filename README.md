...
[![CI](https://github.com/JOJOJONATHANJOSTAR/agent-orchestrator/actions/workflows/ci.yml/badge.svg)](https://github.com/JOJOJONATHANJOSTAR/agent-orchestrator/actions/workflows/ci.yml)
...
需求 ──▶ Claude 规划 ──▶ ┌──────────────── 循环（≤ max-rounds 轮）────────────────┐
                         │ Codex 改代码 ──▶ 验收门(测试/lint) ──▶ 门过才 Claude 评审 │
                         └──── 门全过且评审 pass ? ──是──▶ 完成 / 否──▶ 下一轮 ──────┘
```

> **门过才评审（默认）**：门没过的轮次跳过 Claude 评审（下一轮由门报错驱动），只在门全绿时评审做
> 最终把关——省掉失败轮的低价值评审。可进一步用 `--no-review`（门过即完成，Claude 退出循环）/
> `--no-plan`（跳过规划）让小任务更轻；详见参数表与 SKILL.md 的「小改/极简」档。

## 安装

零运行时依赖（**仅标准库**），Python ≥ 3.10。**跨平台**（Windows / macOS / Linux，CI 三平台验证）。
前置：已安装并登录 `claude`（Claude Code）与 `codex`（Codex CLI）。

> Mac/Linux 提示：`python` 命令可能需写作 `python3`；锁凭据文件用 `chmod 600 ~/.claude_codex_orchestrator.env`
> （Windows 用 `icacls`）。

**A) 作为 CLI 工具安装** —— 拿到全局命令 `agent-orchestrate`：

```bash
# 从 GitHub 直接装（推荐 pipx 隔离环境）
pipx install git+https://github.com/JOJOJONATHANJOSTAR/agent-orchestrator

# 或克隆后安装
git clone https://github.com/JOJOJONATHANJOSTAR/agent-orchestrator
cd agent-orchestrator
pip install .            # 普通安装
pip install -e .         # 可编辑安装（开发用，改动即时生效）
pip install -e ".[dev]"  # 附带 pytest（跑测试用）

# 装好后：
agent-orchestrate "把 user 模块密码改成 bcrypt 并补单测" --repo ../app --gate tests="pytest -q"
agent-orchestrate --help
```

**B) 作为 Claude Code skill 安装** —— 对话式入口 `/agent-orchestrator`，参数由助手自动推断：

```bash
python scripts/deploy.py   # 部署到 ~/.claude/skills/agent-orchestrator（自包含，可分发）
```

skill 随仓库 `orchestrator/` 迭代：改完重跑 `deploy.py` 即可；开发时想「改完即生效免重部署」，
设环境变量 `AGENT_ORCHESTRATOR_HOME` 指向仓库根。
