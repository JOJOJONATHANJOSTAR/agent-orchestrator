...
零运行时依赖（**仅标准库**），Python ≥ 3.10。前置：已安装并登录 `claude`（Claude Code）与
`codex`（Codex CLI）。
...
pipx install git+https://github.com/JOJOJONATHANJOSTAR/agent_corporation_framework

# 或克隆后安装
git clone https://github.com/JOJOJONATHANJOSTAR/agent_corporation_framework
cd agent_corporation_framework
pip install .            # 普通安装
